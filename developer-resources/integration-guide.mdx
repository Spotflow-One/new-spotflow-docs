...
If you don't have an account yet, you can get your business approved by [contacting the founder](https://demo.spotflow.one/) or by filling out this [form](https://spotflow.com/early-access).

## Dashboard Setup

1. Navigate to the [Spotflow Dashboard](https://app.spotflow.one/)
...
4. Generate your API key:
   ...
   - Copy the API key the in env named SPOTFLOW_API_KEY
...
Spotflow supports two types of payment links for integrating payments into your website:
...
For a seamless in-page experience, you can use our **Overlay Checkout** integration, which embeds the Spotflow checkout on your site by leveraging these payment links.
...
<Step title="Construct your payment link">
  ...
  ```text
  https://checkout.spotflow.com/buy/{productid}
  ```
</Step>
...
<Tabs>
  ...
  <Tab title="Node.js SDK">
    ```javascript
    import Spotflow from 'spotflow';

const client = new Spotflow({
  bearerToken: process.env['SPOTFLOW_API_KEY'], // This is the default and can be omitted
});
...
main();
  ...
  <Tab title="Python SDK">
    ```python
    import os
from spotflow import Spotflow

client = Spotflow(
    bearer_token=os.environ.get("SPOTFLOW_API_KEY"),  # This is the default and can be omitted (if using same name `SPOTFLOW_API_KEY`)
)
...
import (
  ...
  "github.com/spotflow/spotflow-go"
  "github.com/spotflow/spotflow-go/option"
)

func main() {
  client := Spotflow.NewClient(
    option.WithBearerToken("My Bearer Token"), // defaults to os.LookupEnv("SPOTFLOW_API_KEY")
  )
  payment, err := client.Payments.New(context.TODO(), spotflow.PaymentNewParams{
    PaymentLink: spotflow.F(true),
    Billing: spotflow.F(spotflow.PaymentNewParamsBilling{
      City: spotflow.F("city"),
      Country: spotflow.F(spotflow.CountryCodeAf),
      State: spotflow.F("state"),
      Street: spotflow.F("street"),
      Zipcode: spotflow.F(int64(0)),
    }),
    Customer: spotflow.F(spotflow.PaymentNewParamsCustomer{
      Email: spotflow.F("email"),
      Name: spotflow.F("name"),
    }),
    ProductCart: spotflow.F([]spotflow.PaymentNewParamsProductCart{spotflow.PaymentNewParamsProductCart{
      ProductID: spotflow.F("product_id"),
      Quantity: spotflow.F(int64(0)),
    }}),
  })
  if err != nil {
    panic(err.Error())
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_SPOTFLOW_TEST_API}/payments`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${process.env.SPOTFLOW_API_KEY}`, // Replace with your API secret key generated from the Spotflow Dashboard
      },
      body: JSON.stringify({
        billing: {
          city: formData.city,
...
You can see all our available SDKs on [Spotflow SDK](developer-resources/spotflow-sdks) page.
...
const webhook = new Webhook(process.env.SPOTFLOW_WEBHOOK_KEY!); // Replace with your secret key generated from the Spotflow Dashboard
...
You can refer to this project with demo implementation on [GitHub](https://github.com/spotflow/spotflow-checkout-demo) using Next.js and TypeScript.

You can check out the live implementation [here](https://atlas.spotflow.com/).